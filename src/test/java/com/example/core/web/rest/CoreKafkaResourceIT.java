package com.example.core.web.rest;
...
import com.example.core.config.KafkaSseConsumer;
import com.example.core.config.KafkaSseProducer;
...
class CoreKafkaResourceIT {
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/core-kafka/publish?message=value-produce")).andExpect(status().isOk());
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/core-kafka/register"))
        ...
        for (int i = 0; i < 100; i++) {
            ...
            if (content.contains("data:value-consume")) {
                restMockMvc.perform(get("/api/core-kafka/unregister"));
                return;
            }
        }
